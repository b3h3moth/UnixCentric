...
/*  Lo scopo del programma e' la modifica di uno o piu' file azzerandoli, 
e successivamente modificare anche il time mediante utime() */
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
       ...
       fprintf(stderr, "Be careful, each file will be truncated to zero.\n");
       exit(EXIT_FAILURE);
    }

    /* La funzione stat() verifica la presenza del file ricevuto in input, 
    in caso di esito positivo la funzione open() lo aprira' in lettura e 
    scrittura, nonche', mediante il flag O_TRUNC, ne azzerera' il contenuto. 
    
    Il file pertanto essendo stato acceduto e modificato, dovrebbe vedersi 
    modificato sia l'orario di accesso sia l'orario di modifica. */
    for (i=1; i<argc; i++) {
