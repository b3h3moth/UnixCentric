...
bool RicercaBinaria(int inf, int sup, TipoVettore A, TipoElemVett elem);
void CreaFile(void);
...
int main(int argc, char *argv[]) {
   ...
   printf("Risultato %d->'%d'\n", cerca, RicercaBinaria(0, n_elementi_letti, vettore, cerca));
...
bool RicercaBinaria(int inf, int sup, TipoVettore A, TipoElemVett elem) {
   ...
   if (inf > sup)
	   ...
	   if(elem == A[med]) {
	   	   ...
	   	   if (elem < A[med])
		   	   trovato = RicercaBinaria(inf, med-1, A, elem);
		   else
		   	   trovato = RicercaBinaria(med+1, sup, A, elem);
	   }
   }
